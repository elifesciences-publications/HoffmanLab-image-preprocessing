...
% parameters:
% folder - specifies the folder containing all the images being
% analyzed
...
doaf = file_search(doa,param.folder);
dodf = file_search(dod,param.folder);
dofr = file_search(dof,param.folder);

if param.nobkgd && nps > 3
    aoaf = file_search(varargin{1},param.folder);
    aodf = file_search(varargin{2},param.folder);
    aofr = file_search(varargin{3},param.folder);
end
...
for i = 1:3
    for j = 1:ndoa
        eval(sprintf('%s{j} = double(imread(fullfile(''%s'',''%s'')));',...
            filecell{i},param.folder,filenames{i}{j}));
    end
end
...
elseif nps >= 6 && param.nobkgd
    for i = 1:3 % read in 4-6 as ao
        for j = 1:length(aoaf)
            eval(sprintf('%s{j} = double(imread(fullfile(''%s'',''%s'')));',...
                filecell{i+6},param.folder,filenames{i+3}{j}))
        end
    end
    daflag = 1;
end
...
for i=1:ndoa
    ...
    if param.ocimg
        if param.nobkgd
            imwrite2tif(dodxdm,[],fullfile(pwd,param.folder,['bs' dodfn{i}]),'single')
            imwrite2tif(dodxdm,[],fullfile(pwd,param.folder,['bs' doafn{i}]),'single')
            imwrite2tif(dodxdm,[],fullfile(pwd,param.folder,['bs' dofrn{i}]),'single')
        else
            imwrite2tif(dodxdm,[],fullfile(pwd,param.folder,['bsff' dodfn{i}]),'single')
            imwrite2tif(dodxdm,[],fullfile(pwd,param.folder,['bsff' doafn{i}]),'single')
            imwrite2tif(dodxdm,[],fullfile(pwd,param.folder,['bsff' dofrn{i}]),'single')
        end
...
if daflag
    for i = 1:length(aoaxam)
        ...
        if param.ocimg
            if param.nobkgd
                imwrite2tif(aodxdm,[],fullfile(pwd,param.folder,['bs' aodfn{i}]),'single')
                imwrite2tif(aoaxdm,[],fullfile(pwd,param.folder,['bs' aoafn{i}]),'single')
                imwrite2tif(aodxam,[],fullfile(pwd,param.folder,['bs' aofrn{i}]),'single')
            else
                imwrite2tif(aodxdm,[],fullfile(pwd,param.folder,['bsff' aodfn{i}]),'single')
                imwrite2tif(aoaxdm,[],fullfile(pwd,param.folder,['bsff' aodfn{i}]),'single')
                imwrite2tif(aodxam,[],fullfile(pwd,param.folder,['bsff' aodfn{i}]),'single')
            end
        end
...
print(fullfile(param.folder,'BT Plot'),'-dpng')
...
if ~isfield(param,'pf')
    save(fullfile(param.folder,'nlabt.dat'),'nlabt','-ascii')
    save(fullfile(param.folder,'nldbt.dat'),'nldbt','-ascii')
    save(fullfile(param.folder,'nlact.dat'),'nlact','-ascii')
    save(fullfile(param.folder,'nldct.dat'),'nldct','-ascii')
else
    if ~isfield(param,'outname')
        param.outname = ' ';
        nele = length(param.pf);
        if nele >= 1
            save(fullfile(pwd,param.folder,[param.pf(1) '_' param.outname]),'nldbt','-ascii')
        end
        if nele >= 2
            save(fullfile(pwd,param.folder,[param.pf(2) '_' param.outname]),'nldct','-ascii')
        end
        if nele >= 3
            save(fullfile(pwd,param.folder,[param.pf(3) '_' param.outname]),'nlabt','-ascii')
        end
        if nele >= 4
            save(fullfile(pwd,param.folder,[param.pf(4) '_' param.outname]),'nlact','-ascii')
        end
    end
end
